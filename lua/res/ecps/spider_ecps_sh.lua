...
function downloadAndParseListPage(pageno) 
	...
	--os.exit(0)
	--local entMl=getEntNames("ecps_sh")
	local entMl={}
	print("上海企业名录长度:",tostring(table.getn(entMl)))
	if table.getn(entMl)==0 then
		...
		local con,_=downloadAdv(true,"https://www.sgs.gov.cn/notice/search/ent_except_list","get",param,head,"")
		print(unGzip(con))	
		if con=="" then 
			print("下载上海异常名录出错")
			return entMl
		...
		for k,v in pairs(listHtml) do
			if k>1 and k<table.getn(listHtml) then
				...
				item["href"]="http://www.sgs.gov.cn/notice/captcha?preset=&ra=0.1800365946930833" --验证码
