...
spiderStoreMode=2
spiderStoreToMsgEvent=4102 --消息总线event
...
--默认列表页第一页
spiderTargetChannelUrl="http://law.npc.gov.cn/FLFG/index/xingfamore.jsp"

--取得对方网站最后发布时间 必须返回yyyy-MM-dd HH:mm:ss 格式
function getLastPublishTime()
	local content = download(spiderTargetChannelUrl,{})
	local tmp = findOneText("#main table:eq(0) tbody:eq(0) tr:eq(1) td:eq(2)",content)
...
function downloadAndParseListPage(pageno)
	...
	local list = findListHtml("#main table:eq(0) tbody:eq(0) tr",content)
...
function downloadDetailPage(data)
	for i=1,5 do 	--5次下载任务不成功，退出
		...
		data["issuefile"]=findOneText("#content table:eq(1) tr:eq(2) td:eq(2)",con)
		data["issuefile"]=com.trim(data["issuefile"])
		data["issuedate"]=findOneText("#content table:eq(1) tr:eq(3) td:eq(1)",con)
		data["issuedate"]=com.trim(data["issuedate"])
