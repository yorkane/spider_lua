...
--默认列表页第一页
spiderTargetChannelUrl="http://www.atobo.com.cn/Companys/s-p1-s870"
local area="北京";
...
function downloadAndParseListPage(pageno)
	...
	for k,v in pairs(list) do
			...
			if pageno~=1 and k==1 then
				if prevList==item["title"] then
					print("有重复页page：",pageno)
					return
				end
			end
			if k==1 then
				prevList=item["title"]
				print(item["title"])
				print("najisuehihenfnishjjjwjojiushmacbookair")
			end
