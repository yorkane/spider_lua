...
--默认列表页第一页
spiderTargetChannelUrl="http://law.npc.gov.cn/FLFG/getAllList.action?SFYX=%E6%9C%89%E6%95%88&zlsxid=&bmflid=&zdjg=&txtid=64&resultSearch=false&lastStrWhere=&keyword=&pagesize=20"


local f=io.open("url.txt","r")
local fw=io.open("urlw.txt","a")
--取得对方网站最后发布时间 必须返回yyyy-MM-dd HH:mm:ss 格式
function getLastPublishTime()

	spiderTargetChannelUrl=f:read() --读取URL
	spiderMaxPage=tonumber(f:read()) --读取最大下载页
	if spiderTargetChannelUrl~=nil and  spiderMaxPage~=nil then
		print("url:",spiderTargetChannelUrl,spiderMaxPage)
		--os.exit()
		fw:write(spiderTargetChannelUrl.."\n")
...
function downloadAndParseListPage(pageno)
	...
	print("pageno:",pageno)
	print("url",spiderTargetChannelUrl)
