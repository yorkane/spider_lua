...
--名称 网站名称_栏目
spiderName="中国建设招标网_招标采购公告";
--代码 区域代码_网站代码_栏目代码
spiderCode="a_zgjszbw_zbcggg";
--是否下载3级页
...
spiderLastDownloadTime="2016-06-23 16:09:01";
--执行频率30分钟
spiderRunRate=20;
...
--默认列表页第一页
spiderTargetChannelUrl="http://www.jszhaobiao.com/search.html?scope=1&dt=7&btns=1&page=1"
...
function downloadAndParseListPage(pageno)
	...
	if pageno==1 then
		...
		href="http://www.jszhaobiao.com/search.html?scope=1&dt=7&btns=1&page="..tostring(pageno)
	end
	...
	for k,v in pairs(list) do
		if (k==1 or k==2 or k==3 or k==4 or k==5) and pageno==1 then
		else
			if k==1 then
			else
				local item = {}
				...
				tmpstr=""
				--print(item["title"],item["href"],item["publishtime"])
				--os.exit()
				if k==6 and pageno==1 then
