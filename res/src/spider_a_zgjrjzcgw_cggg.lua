...
--名称 网站名称_栏目
spiderName="中国金融集中采购网_采购公告";
--代码 区域代码_网站代码_栏目代码
spiderCode="a_zgjrjzcgw_cggg";
--是否下载3级页
...
--默认列表页第一页
spiderTargetChannelUrl="http://www.cfcpn.com/plist/caigou"
...
function downloadDetailPage(data)
	for i=1,5 do 	--5次下载任务不成功，退出
		...
		data["detail"]=findOneText(".notice_describe",con)
		data["contenthtml"]=findOneHtml(".notice",con)
		data["buyer"]=findOneText(".notice_information p:eq(0) span:eq(0)",con)
		data["buyer"]=com.split(data["buyer"]," ")[1]
		data["buyer"]=string.sub(data["buyer"],13,string.len(data["buyer"]))
		--print(data["buyer"])
		--os.exit()
