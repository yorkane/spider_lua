...
--名称 网站名称_栏目
spiderName="广东省政府采购网_电子反拍";
--代码 区域代码_网站代码_栏目代码
spiderCode="gd_gdszfcgw_gzfp";
--是否下载3级页
...
spiderTargetChannelUrl="http://www.gdgpo.gov.cn/queryMoreInfoList.do?channelCode=0017&sitewebId=-1"
...
function downloadAndParseListPage(pageno)
	...
	if pageno==1 then
		...
		href="http://www.gdgpo.gov.cn/queryMoreInfoList.do?channelCode=0017&pageSize15&pointPageIndexId=9&sitewebId=-1&pageIndex="..tostring(pageno)
	end
	...
	for k,v in pairs(list) do
		if (k==1 or k==2) and pageno==1 then
			...
			local item = {}
			item["title"]=com.trim(findOneText("a:eq(2):attr(title)",v))
			item["city"]=findOneText("a:eq(1)",v)
			item["href"]=findOneText("a:eq(2):attr(href)",v)
