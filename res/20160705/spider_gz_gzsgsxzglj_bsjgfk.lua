...
--名称 网站名称_栏目
spiderName="贵州省工商行政管理局_办事结果反馈";
--代码 区域代码_网站代码_栏目代码
spiderCode="gz_gzsgsxzglj_bsjgfk";
--是否下载3级页
...
--默认列表页第一页
spiderTargetChannelUrl="http://www.gzaic.org.cn/ywlcList.jsp"
local trylistnum=0 --防止死循环
--取得对方网站最后发布时间 必须返回yyyy-MM-dd HH:mm:ss 格式
function getLastPublishTime()
	local content = download(spiderTargetChannelUrl,{})
	local tmp = findOneText(".gbTableW tbody:eq(0) tr:eq(1) td:eq(3)",content)
...
function downloadAndParseListPage(pageno)
	...
	local list = findListHtml(".gbTableW tr",content)
	...
	for k,v in pairs(list) do
		if k==1 then
			...
			--print(item["enterprisename"],item["publishtime"])
			tmpstr=""
