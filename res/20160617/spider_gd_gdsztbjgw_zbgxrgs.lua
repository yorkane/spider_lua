...
--名称 网站名称_栏目
spiderName="广东省招投标监管网_中标候选人公示";
--代码 区域代码_网站代码_栏目代码
spiderCode="gd_gdsztbjgw_zbgxrgs";
--是否下载3级页
...
spiderTargetChannelUrl="http://www.gdzbtb.gov.cn/zbhxrgsbd/"
...
function downloadAndParseListPage(pageno)
	...
	if pageno==1 then
		...
		href="http://www.gdzbtb.gov.cn/zbhxrgsbd/index_"..tostring(pageno-1)..".htm"
	end
...
function downloadDetailPage(data)
	for i=1,5 do 	--5次下载任务不成功，退出
		...
		data["site"]="广东省招投标监管网"
		data["channel"]="中标候选人公示"
		data["toptype"]="结果"
		data["subtype"]="中标"
		data["type"]="bid"
		data["spidercode"]=spiderCode
		data["title"]=com.trim(findOneText(".cn03:eq(0)",con))
		data["detail"]=findOneText("body>table:eq(2) table:eq(4)",con)
		data["contenthtml"]=findOneHtml("body>table:eq(2) table:eq(4)",con)
